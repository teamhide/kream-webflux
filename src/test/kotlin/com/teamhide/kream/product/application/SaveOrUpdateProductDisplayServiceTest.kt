...
internal class SaveOrUpdateProductDisplayServiceTest(
    ...
    Given("전시 상품 정보가 존재하지 않고 상품도 존재하지 않을 때") {
        ...
        When("저장 또는 업데이트 요청을 하면") {
            ...
            Then("저장하지 않는다") {
                productDisplayRepository.findAll().toList().size shouldBe 0
            }
        }
    }
    ...
    Given("전시 상품 정보가 존재하지 않을 때") {
        ...
        val product = productRepository.save(makeProduct(productBrandId = productBrand.id, productCategoryId = productCategory.id))

        val command = makeSaveOrUpdateProductDisplayCommand(productId = product.id)
    ...
    Given("전시 상품 정보가 존재하고 새로운 가격보다 저렴한 경우") {
        ...
        val command = makeSaveOrUpdateProductDisplayCommand(productId = productDisplay.productId, price = 20000)
